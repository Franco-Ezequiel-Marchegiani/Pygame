...
def mostrar_juego(pantalla: pg.Surface, cola_eventos: list[pg.event.Event], datos_juego: dict) -> tuple:
    ...
    #Todo esto TIENE que ser una función. Para que tenga "manejo de eventos", con los params que necesite
        ...
        if evento.type == pg.QUIT:
            ...
            if lista_cartas_dictionary and\
                ...
                not lista_cartas_dictionary[-1].get('visible'):
                    ...
                    cuadro_frase['superficie'].fill(var.COLOR_NEGRO) #Para tapar las frases anteriores

                    print(f"datos_juego: {datos_juego}")
                    print(f"lista_cartas_vistas: {lista_cartas_vistas}")
                    print("Espacio")
                    print(f"carta_vista: {carta_vista}")
                    print(f"carta_vista.get('puntaje'): {carta_vista.get('puntaje')}")
                    aux.actualizar_puntaje(datos_juego, lista_cartas_vistas[-1].get('puntaje')) #O, lista_cartas_vistas[-1].get('puntaje')
    ...
    aux.mostrar_texto(cuadro_puntaje['superficie'], f"Score: {datos_juego.get('puntaje')}", (20,20), var.FUENTE_20, var.COLOR_ROJO)
    ...
    #Este fragmento pasarlo a una función

    
    #Blit para mostrar y renderizar el cuadro
    ...
    #Para direccionar al usuario al menú cuando ya no hayan cartas
    if not lista_cartas_dictionary and not datos_juego.get('tiempo_finalizado'): #Cartas sin abrir, originales
